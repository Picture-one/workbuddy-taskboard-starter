  ...
  for (const [key, storedValue] of projectBoardDisplaySettingsStorageEntries()) {
    ...
    try {
      ...
      if (value && typeof value === "object" && !Array.isArray(value)) {
        settings[projectId] = normalizeBoardDisplaySettings(value);
      }
    } catch {
...
export function App() {
  ...
  // 对话宿主：默认 WorkBuddy（本机 workbuddy:// 已注册，codex:// 未注册）。
  // 可用 ?conversationApp=codex 退回上游行为。
  const conversationApp = resolveConversationApp(query);
  ...
  /**
   * 列内按天分组（纯展示层变换，**不改变 `tasks` / `tasksByStatus` 的顺序**）。
   *
   * ⚠️ 为什么刻意不在渲染前重排数组：`moveTask()`（`:2540-2551`）从 **`tasks`** 取 `destination`
   * 并用 `findIndex(beforeTaskId)` 算插入位，而 `beforeTaskId` 来自按**真实 DOM 顺序**扫描的
   * `findDropBefore()`。若这里按天重排了数组，两套下标就会错位。保持数组顺序不变时，
   * 每个「天分组」内部仍是 `sortTasks` 的 `sort_order` 序 → **同一天内拖拽落点精确**。
   * 已知限制：同列内跨天拖拽后，卡片会回到自己的天分组（天数由日期决定，与拖拽位置无关）。
   */
  const groupsByStatus = useMemo(() => {
    if (!boardDisplaySettings.groupByDay) return null;
    const dayText: DayGroupingText = {
      today: text("今天", "Today"),
      yesterday: text("昨天", "Yesterday"),
      undated: text("未标注日期", "No date"),
    };
    return Object.fromEntries(
      TASK_STATUSES.map((status) => [status, groupTasksByDay(tasksByStatus[status], locale, dayText)]),
    ) as Record<TaskStatus, DayGroup[]>;
  }, [boardDisplaySettings.groupByDay, locale, tasksByStatus, text]);
  ...
  // 「今日推送」是看板里的普通一列（与状态列同一套结构与样式），占一个网格位，
  // 因此必须计入列数，否则 .board 的网格会少一列、板块被挤窄。
  const showTodayColumn = boardDisplaySettings.showTodayPanel;
  const mainColumnCount = Math.max(mainBoardItems.length + (showTodayColumn ? 1 : 0), 1);
  ...
  function reportProtocolMissing(app: ConversationApp) {
    setActionError(app === "workbuddy"
      ? text(
        "未能唤起 WorkBuddy 新建对话。请确认 WorkBuddy 已安装，且已注册 workbuddy:// 协议。",
        "Could not open a new WorkBuddy conversation. Make sure WorkBuddy is installed and the workbuddy:// protocol is registered.",
      )
      : text(
        "未能唤起 Codex 新建对话。请确认 Codex 已安装，且已注册 codex:// 协议。",
        "Could not open a new Codex thread. Make sure the Codex app is installed and the codex:// protocol is registered.",
      ));
  }

  function buildConversationInstruction(label: string) {
    return text(
      `[$manage-taskboard](${manageTaskboardSkillPath}) 继续处理任务面板中的这项工作：${label}`,
      `[$manage-taskboard](${manageTaskboardSkillPath}) Continue this taskboard item: ${label}`,
    );
  }

  /**
   * 打开对话的唯一出口。默认在 WorkBuddy 里**新开一条对话**（草稿态，用户确认后提交）；
   * 仅在显式选择 codex 宿主且已知旧 threadId 时，才回到上游的「打开既有 Codex thread」行为。
   */
  function openConversationTarget(options: {
    instruction?: string;
    label?: string;
    workspacePath?: string;
    existingThreadId?: string;
  }) {
    const { instruction, label, workspacePath, existingThreadId } = options;
    if (conversationApp === "codex" && existingThreadId) {
      openExternalUrl(buildCodexThreadUrl(existingThreadId), () => reportProtocolMissing("codex"));
      return;
    }
    const prompt = instruction ?? buildConversationInstruction(label ?? existingThreadId ?? "");
    const url = conversationApp === "codex"
      ? buildCodexNewThreadUrl(prompt, workspacePath)
      : buildWorkBuddyNewTaskUrl(prompt, workspacePath);
    openExternalUrl(url, () => reportProtocolMissing(conversationApp));
  }

  function openThread(binding: CodexThreadBinding, threadLabel?: string) {
    ...
    if (binding.codexProjectKind === "remote") {
      setActionError(text(
        "该对话绑定的是 SSH 远程项目，无法在本地新开对话。请在远程主机上继续处理。",
        "This conversation is bound to an SSH remote project and cannot be opened locally. Continue on the remote host.",
      ));
      return;
    }
    openConversationTarget({
      label: threadLabel ?? binding.threadId,
      workspacePath: binding.workspacePath,
      existingThreadId: binding.threadId,
    });
  }

  function openLegacyLocalThread(threadId: string, threadLabel?: string) {
    if (embedded && window.parent !== window) {
      postEmbeddedHostMessage({
        type: "taskboard:open-thread",
    ...
    openConversationTarget({ label: threadLabel ?? threadId, existingThreadId: threadId });
  }
  ...
  async function openTaskInThread(task: Task) {
    ...
    if (standalone) {
      if (codexProjectContext?.codexProjectKind === "remote") {
        setActionError(text(
          "该任务绑定的是 SSH 远程项目，无法在本地新开对话。请在远程主机上继续处理。",
          "This task is bound to an SSH remote project and cannot be opened locally. Continue on the remote host.",
        ));
        return;
      }
      openConversationTarget({
      ...
      <main className="workspace">
        ...
        {detailTask && selectedProject ? (
          ...
          <div
            ...
            data-main-columns={mainBoardItems.length + (showTodayColumn ? 1 : 0)}
            style={{
            ...
            {tasksLoading && !hasLoadedTasks ? (
              ...
              <>
                <div ref={boardScrollRef} className="board-scroll" aria-label={text("议题看板", "Issue board")}>
                  <div className="board">
                    {/* 「今日推送」是看板的第一列：与状态列共用 .board-column 结构与样式，
                        只放带 daily-card 标签的卡片，只读、不参与拖拽。 */}
                    {showTodayColumn && (
                      <TodayPushPanel
                        tasks={filteredTasks}
                        presentations={taskPresentations}
