...
import type { DayGroup } from "../boardGrouping";
...
export function BoardColumn({
  ...
  /**
   * 卡片渲染。**按天分节与平铺共用同一个函数**，确保两条分支产出的卡片属性完全一致
   * （差别只在外面套不套 `column-day-header`）。
   */
        ...
        {dayGroups && dayGroups.length > 0
          ? dayGroups.map((group) => (
              <Fragment key={group.key}>
                {/* 分节标题刻意不携带 data-task-id —— findDropBefore() 只查 [data-task-id]，
                    因此标题被自然排除在落点判定之外，卡片 DOM 序仍严格等于 group.tasks 拼接序。 */}
                <div className="column-day-header" data-day-section={group.key}>
                  <span className="column-day-label">{group.label}</span>
                  <span className="column-day-count">{group.count}</span>
                </div>
