...
import { conversationAppDisplayName, resolveConversationApp, type ConversationApp } from "../conversationTarget";
...
function conversationStatus(
  ...
  return conversation.kind === "local-ai" ? text("已暂停", "Paused") : conversationAppDisplayName(app);
}
...
export function TaskConversationMenu({
  ...
  const conversationApp = useMemo(
    () => resolveConversationApp(new URL(document.baseURI).searchParams),
    [],
  );
          ...
          {conversations.map((conversation) => (
            ...
            >
              ...
              <span className={`task-conversation-menu-status${conversation.currentRun?.status === "running" ? " is-running" : ""}`}>
                {conversationStatus(conversation, text, conversationApp)}
              </span>
            </button>
          ))}
        </div>,
