...
const conversationTarget = await readFile(new URL("../web/src/conversationTarget.ts", import.meta.url), "utf8");
...
test("the standalone web page opens linked conversations through the configured app deep link", () => {
  // 单页版不再把 codex:// 硬编码在 App.tsx —— 一律经 openConversationTarget 出海，
  // 默认宿主是 WorkBuddy（workbuddy://task?action=start&prompt=&cwd=）。
  assert.match(webApp, /function openConversationTarget\(options: \{/);
  assert.match(webApp, /openExternalUrl\(buildCodexThreadUrl\(existingThreadId\)/);
  assert.match(webApp, /buildWorkBuddyNewTaskUrl\(prompt, workspacePath\)/);
  assert.match(webApp, /existingThreadId: binding\.threadId/);
  assert.doesNotMatch(webApp, /window\.location\.assign\(`codex:\/\/threads\//);
  // 深链形状由单一真源模块产出（避免再次出现"多处硬编码、改一处漏两处"）
  assert.match(conversationTarget, /new URL\("workbuddy:\/\/task"\)/);
  assert.match(conversationTarget, /searchParams\.set\("action", "start"\)/);
  assert.match(conversationTarget, /MAX_DEEPLINK_PROMPT = 8000/);
  assert.match(conversationTarget, /app === "codex" \? "Codex" : "WorkBuddy"/);
});
...
test("remote Codex tasks wait for the exact project and host without a local route fallback", () => {
  ...
  assert.match(webApp, /openThread\(conversation\.threadBinding, conversation\.title\)/);
...
test("the standalone web page always opens a project-scoped new conversation", () => {
  ...
  // standalone 分支改为经 openConversationTarget 新开对话（不再自带 codex:// 深链）
  assert.match(webApp, /openConversationTarget\(\{\n\s*instruction: embeddedInstruction,\n\s*workspacePath,\n\s*\}\)/);
  assert.match(conversationTarget, /new URL\("codex:\/\/threads\/new"\)/);
  assert.match(conversationTarget, /searchParams\.set\("path", cwd\)/);
  assert.match(conversationTarget, /searchParams\.set\("prompt", clampPrompt\(prompt\)\)/);
});
